...
//! This is a critical feature for production systems where documents change
//! frequently.
...
///
/// Controls how the graph manager handles new content, including change
/// detection, confidence thresholds, batching, and conflict resolution
/// strategies.
///
/// # Examples
///
...
pub struct IncrementalConfig {
    ...
    ///
    /// When enabled, the system computes content hashes to avoid redundant
    /// updates for unchanged documents, improving performance in production
    /// systems.
    ...
    ///
    /// Prevents memory exhaustion and ensures responsive updates. Larger
    /// batches improve throughput but increase memory usage and latency.
    ...
    /// existing graph data (e.g., different attribute values for the same
    /// entity).
...
/// Strategy for resolving conflicts when updating existing nodes
///
/// When the same entity or relationship is encountered with different
/// attributes, this strategy determines how to merge or select the final
/// values.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ConflictResolution {
    /// New data always overwrites existing data
    ///
    /// Simple and fast, suitable when newer information is consistently more
    /// accurate. Loses historical data unless versioning is enabled
    /// separately.
    LatestWins,
    ...
    ///
    /// Updates fail with an error, allowing external systems to review
    /// conflicts. Use this in high-stakes domains where accuracy is
    /// critical.
    Manual,
}
...
///
/// Nodes are versioned and timestamped to support incremental updates,
/// rollbacks, and change tracking. Each node can store semantic embeddings for
/// similarity search.
///
/// # Examples
///
...
pub struct GraphNode {
    ...
    /// Typically generated by embedding models like BERT or sentence
    /// transformers.
...
///
/// Different node types enable type-specific query strategies and graph
/// traversal patterns.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum NodeType {
    /// A named entity extracted from text (person, organization, location,
    /// etc.)
    ///
    ...
    ///
    /// Chunks enable retrieval at sub-document granularity for better
    /// precision.
    Chunk,
...
///
/// Edges represent relationships, containment, references, or similarity
/// between nodes. Each edge has a weight indicating strength or confidence of
/// the relationship.
///
/// # Examples
///
...
///     attributes: HashMap::from([(
///         "context".to_string(),
///         "co-occurrence in paragraph 3".to_string(),
///     )]),
///     created_at: chrono::Utc::now(),
/// };
/// ```
...
pub struct GraphEdge {
    ...
    ///
    /// Can store contextual information like source sentences, confidence
    /// breakdowns, etc.
    pub attributes: HashMap<String, String>,
...
///
/// Different edge types enable structured graph queries and relationship
/// reasoning.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum EdgeType {
    /// General semantic relationship between entities or concepts
    ///
    /// Used when entities co-occur, interact, or share context without a more
...
///
/// Tracks all changes to the graph for debugging, rollback, and compliance
/// purposes. Each update operation generates a record that captures what
/// changed and when.
///
/// # Examples
///
...
///     metadata: HashMap::from([("source".to_string(), "document_42".to_string())]),
/// };
/// ```
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UpdateRecord {
    ...
    ///
    /// Can include source document IDs, user identifiers, or processing
    /// metrics.
...
///
/// Enables precise tracking and selective rollback of different operation
/// types.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum UpdateType {
...
impl IncrementalGraphManager {
    ...
    ///
    /// Initializes an empty knowledge graph with support for concurrent
    /// updates, change detection, and versioned history tracking.
    ///
    /// # Arguments
    ///
    /// * `config` - Configuration controlling update behavior, conflict
...
///
/// Represents a document to be processed and integrated into the knowledge
/// graph. The system will extract entities, relationships, and concepts from
/// the text.
///
/// # Examples
///
...
pub struct DocumentContent {
    ...
    ///
    /// Used for change detection and deduplication. Should be stable across
    /// updates.
    pub id: String,
    ...
    ///
    /// Can include source, author, creation date, tags, or domain-specific
    /// fields.
    pub metadata: HashMap<String, String>,
}
...
///
/// Allows selective updates to node fields without requiring the complete node
/// data. Only fields that are `Some(_)` will be updated; `None` fields are left
/// unchanged.
///
/// # Examples
///
...
///
/// Provides metrics for monitoring graph growth, update performance, and data
/// quality. Returned by update operations to indicate what changed.
///
/// # Examples
///
/// ```
...
///
/// println!(
///     "Added {} new entities in {}ms",
///     summary.nodes_added, summary.time_taken_ms
/// );
/// ```
...
pub struct UpdateSummary {
    ...
    ///
    /// Includes entity extraction, conflict resolution, and graph
    /// modifications.
...
///
/// Provides high-level metrics about graph size and activity for monitoring and
/// debugging.
///
/// # Examples
///
...
mod tests {
    ...
    fn test_incremental_update() {
        ...
        assert_eq!(summary.nodes_added, 0); // No entities extracted from simple
                                            // text
    }
