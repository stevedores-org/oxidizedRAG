...
use graphrag_core::core::{ChunkingStrategy, Document, DocumentId, KnowledgeGraph};
...
/// Index a set of fixture files into a GraphRAG knowledge graph.
///
/// Returns the knowledge graph with documents chunked and added.
fn index_fixtures(filenames: &[&str], chunk_size: usize) -> Result<KnowledgeGraph> {
...
mod performance {
    ...
    fn bench_indexing_speed() {
        ...
        // Indexing 3 small files should complete in under 5 seconds
        assert!(
            elapsed.as_secs() < 5,
    ...
    fn bench_query_latency() {
        ...
        // p99 should be under 500ms for small codebases
        assert!(
            p99.as_millis() < 500,
    ...
    fn bench_tree_sitter_chunking_speed() {
        ...
        // Tree-sitter should parse in under 10ms per file
        assert!(
