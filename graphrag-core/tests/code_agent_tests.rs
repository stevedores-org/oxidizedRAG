...
// ---------------------------------------------------------------------------
// Module 6: Performance Benchmarks
// ---------------------------------------------------------------------------
...
mod performance_benchmarks {
    ...
    #[test]
    fn test_indexing_speed_baseline() {
        ...
        assert!(
            ...
            "Indexing performance regression: {}ms > {}ms",
            elapsed,
    ...
    #[test]
    fn test_query_latency_baseline() {
        let graph = build_graph_from_fixtures(&["calculator.rs"])
            .expect("Failed to build graph");
        ...
        assert!(
            ...
            "Query latency regression: {}ms > {}ms",
            elapsed,
            QUERY_THRESHOLD_MS
        );
    }
    ...
    #[test]
    fn test_throughput_files_per_second() {
        let start = Instant::now();
        let file_count = 3;
        ...
        assert!(
            throughput > 0.5,
            "Throughput too low: {:.2} files/sec < 0.5",
            throughput
        );
    }

    #[test]
    fn test_p99_query_latency() {
        let graph = build_graph_from_fixtures(&["calculator.rs"])
            .expect("Failed to build graph");
        ...
        let p99 = latencies.get(p99_index.saturating_sub(1)).copied().unwrap_or(0);

        println!("P99 query latency: {}ms", p99);

        assert!(
            p99 < QUERY_THRESHOLD_MS * 2,
            "P99 latency too high: {}ms",
            p99
        );
    }
    ...
    fn test_memory_efficiency() {
        ...
        println!(
            "Memory efficiency: {:.2} chunks/MB",
            chunks_per_mb
        );

        assert!(
            chunks_per_mb >= 2.0,
            "Chunking granularity too coarse: {:.2}",
            chunks_per_mb
        );
    }
}
